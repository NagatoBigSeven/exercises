...
class TreeOpsTests extends munit.FunSuite:
  ...
  // Sample tree with 6 nodes and depth 3
  val sampleTree2: IntTree =
    IntBranch(
      4,
      IntBranch(2, IntBranch(1, IntEmptyTree(), IntEmptyTree()), IntBranch(3, IntEmptyTree(), IntEmptyTree())),
      IntBranch(6, IntBranch(5, IntEmptyTree(), IntEmptyTree()), IntBranch(7, IntEmptyTree(), IntEmptyTree()))
    )
  ...
  test("treeShow: sample trees"):
    assertEquals(treeShow(sampleTree), "1\n2\n3\n")
    assertEquals(treeShow(sampleTree2), "4\n2\n1\n3\n6\n5\n7\n")

  test("treeShowPostOrder: sample trees"):
    assertEquals(treeShowPostOrder(sampleTree), "2\n3\n1\n")
    assertEquals(treeShowPostOrder(sampleTree2), "1\n3\n2\n5\n7\n6\n4\n")

  test("treeShowInOrder: sample trees"):
    assertEquals(treeShowInOrder(sampleTree), "2\n1\n3\n")
    assertEquals(treeShowInOrder(sampleTree2), "1\n2\n3\n4\n5\n6\n7\n")
