...
class ListOpsTests extends munit.FunSuite:
  ...
  def toSet[A](l: MyList[A]): Set[A] =
    l match
      case Nil        => Set()
      case Cons(h, t) => toSet(t) + h
  ...
  test("distinctPairs: permutations of 'abc'"):
    val res = distinctPairs(Cons('a', Cons('b', Cons('c', Nil))))
    assertEquals(
      toSet(res),
      Set(('a', 'b'), ('a', 'c'), ('b', 'a'), ('b', 'c'), ('c', 'a'), ('c', 'b'))
    )

  test("distinctPairs: permutations of ''"):
    assertEquals(distinctPairs(Nil), Nil)

  test("distinctPairs: permutations of AaBb"):
    val res = distinctPairs(Cons('A', Cons('a', Cons('B', Cons('b', Nil)))))
    val expected = Set(
      ('A', 'a'),
      ('A', 'B'),
      ('A', 'b'),
    ...
    assertEquals(toSet(res), expected)
